"""
Script de démonstration autonome avec simulations de trading réelles
"""
...
data_collector = load_module_from_file('data_collector', os.path.join(core_path, 'data_collector.py'))
technical_analysis = load_module_from_file('technical_analysis', os.path.join(core_path, 'technical_analysis.py'))
trading_bot_simple = load_module_from_file('trading_bot_simple', os.path.join(core_path, 'trading_bot_simple.py'))

DataCollector = data_collector.DataCollector
TechnicalAnalysis = technical_analysis.TechnicalAnalysis
SimpleTradingBot = trading_bot_simple.SimpleTradingBot
...
def main():
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] == 'single':
            ...
            print("Usage: python demo_trading_standalone.py [single] [symbol]")
    else:
